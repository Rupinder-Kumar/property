...
export default function VendorPropertyForm() {
...
const handleDateChange = (e) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
	  if (!formData.propertyType || !formData.location || !formData.propertyName || !formData.saleableArea || !formData.propertyPrice || !formData.propertyStatus || !formData.possessionDate || !formData.transactionType || !formData.description || !file) {
  ...
  console.log('Combined FormData:', combinedFormData);
    try {
      const response = await fetch('http://localhost:4300/vendor-submit-form', {
        method: 'POST',
  ...
  <h3>VENDOR PROPERTY DASHBARD</h3>
  <div className="row">
...
{showSuccess && (
		...
		Vendor Property added successfully
		</div>
		)}
		{showError && (
		<div className="text-danger mt-2">
